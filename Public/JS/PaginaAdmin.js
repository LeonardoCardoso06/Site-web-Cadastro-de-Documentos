...
function renderUsuarioRow(user) {
 ...
 <td class="delete-column">
  <button class="delete-btn" onclick="excluirUsuario(${user.id})">🗑️</button>
</td>
  `;
...
async function abrirModalInfo(user) {
  try {
    ...
    infoContainer.innerHTML = `
      ...
      <p><strong>VT:</strong> ${dados.vale_transporte ? "Sim" : "Não"}</p>
      <p><strong>Nº Trem:</strong> ${dados.numero_trem || "-"}</p>
      <p><strong>Nº Ônibus:</strong> ${dados.numero_onibus || "-"}</p>
    ...
    if (!dados.arquivos || dados.arquivos.length === 0) {
      ...
      dados.arquivos.forEach((doc) => {
        ...
        div.innerHTML = `
          ...
          <button class="painel-button" onclick="visualizarDocumento('${url}', '${doc.document_type}')">👁️ Visualizar</button>
